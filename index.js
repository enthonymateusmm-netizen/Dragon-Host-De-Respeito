...
function isOwnerOrAdmin(interaction) {
  if (process.env.DONOS.includes(interaction.user.id)) return true;
...
function startApp(appId) {
  ...
  child.on('close', code => {
    ...
    if (process.env.KEEP_ALIVE && desiredOnline.get(appId)) {
      addLog(appId, `Auto-restart ativado. Reiniciando em ${process.env.RESTART_TIME || 3000}ms...`);
      setTimeout(() => {
        if (!running.has(appId) && desiredOnline.get(appId)) {
          try { startApp(appId); } catch (err) { addLog(appId, `[AUTO-RESTART ERRO] ${err.message}`); }
        }
      }, process.env.RESTART_TIME || 3000);
    }
  });
}
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isButton() && interaction.customId === 'host:create') {
      const apps = loadApps();
      if (apps.length >= process.env.MAX_APPS) return interaction.reply({ content: `Limite máximo de ${process.env.MAX_APPS} aplicações atingido.`, ephemeral: true });
...
client.on('messageCreate', async message => {
  ...
  if (!process.env.DONOS.includes(message.author.id) && !message.member?.permissions.has(PermissionFlagsBits.Administrator)) return;
  ...
  const maxZipBytes = Number(process.env.MAX_GB || 250) * 1024 * 1024 * 1024;
  ...
  try {
    if (attachment) {
      ...
      if (attachment.size > maxZipBytes) return message.reply(`O ZIP passou de ${process.env.MAX_GB || 250}GB.`);
      addLog(app.id, `Recebendo ZIP via Discord: ${attachment.name}`);
      await downloadFile(attachment.url, zipPath, maxZipBytes, app.id);
    } else {
      if (!process.env.PERMITIR_LINKS) return message.reply('Links externos estão desativados no `config.json`.');
